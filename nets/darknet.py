...
class CSPLayer(nn.Module):
    def __init__(self, in_channels, out_channels, n=1, shortcut=True, expansion=0.5, depthwise=False, act="silu",):
        ...
        super().__init__()
        hidden_channels = int(out_channels * expansion)
        self.conv1  = BaseConv(in_channels, hidden_channels, 1, stride=1, act=act) #先卷积再经过残差卷积，然后与卷积后的特征图堆叠，再卷积调整通道数，作为一个特征提取层，输入输出的通道数和尺寸均不变
...
class CSPDarknet_SADC(nn.Module):
    def __init__(self, dep_mul, wid_mul, out_features=("dark3", "dark4", "dark5"), depthwise=False, act="silu",):
        super().__init__()
        assert out_features, "please provide output features of Darknet"
        self.out_features = out_features
        ...
        self.dark2 = nn.Sequential(
            Conv(base_channels, base_channels * 2, 3, 2, act=act),
            CSPLayer(base_channels * 2, base_channels * 2, n=base_depth, depthwise=depthwise, act=act),
            SADCBlock(base_channels * 2, base_channels * 2,kernel_size=3),
        )

        self.dark3 = nn.Sequential(
            Conv(base_channels * 2, base_channels * 4, 3, 2, act=act),
            CSPLayer(base_channels * 4, base_channels * 4, n=base_depth * 3, depthwise=depthwise, act=act),
            SADCBlock(base_channels * 4, base_channels * 4, kernel_size=3),
        )
        self.swt1 = C3TR(512, 512)
        ...
        self.dark4 = nn.Sequential(
            Conv(base_channels * 4, base_channels * 8, 3, 2, act=act),
            CSPLayer(base_channels * 8, base_channels * 8, n=base_depth * 3, depthwise=depthwise, act=act),
            SADCBlock(base_channels * 8, base_channels * 8,kernel_size=3),
        ...
        self.dark5 = nn.Sequential(
            ...
            SPPBottleneck(base_channels * 16, base_channels * 16, activation=act),
            CSPLayer(base_channels * 16, base_channels * 16, n=base_depth, shortcut=False, depthwise=depthwise, act=act),
            SADCBlock(base_channels * 16, base_channels * 16, kernel_size=3)
        )
        self.deconv = DeformConv2D(512, 512, kernel_size=3, padding=1, modulation=True)
...
class SADCBlock(torch.nn.Module):
    def __init__(self,
                 in_channels,
                 out_channels,
                 kernel_size,
        ...
        super(SADCBlock, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.kernel_size = kernel_size
        self.stride = stride
        self.offset_conv1 = nn.Conv2d(in_channels, 3*kernel_size*kernel_size*(in_channels//groups),
                                      3, stride=1, padding=1, bias=False)
        self.SA = SpatialAttention()
        self.deform1 = DeformConv2d(in_channels, out_channels, kernel_size =3, padding=1, groups=groups)
