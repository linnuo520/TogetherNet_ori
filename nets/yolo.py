...
from .darknet import BaseConv, CSPDarknet,CSPDarknet_SADC, CSPLayer, DWConv, SCBottleneck
...
class YOLOPAFPN(nn.Module):
    def __init__(self, model_type,depth = 1.0, width = 1.0, in_features = ("dark3", "dark4", "dark5"), in_channels = [256, 512, 1024], depthwise = False, act = "silu"):
        super().__init__()
        Conv                = DWConv if depthwise else BaseConv
        if model_type == "ori":
            self.backbone       = CSPDarknet(depth, width, depthwise = depthwise, act = act)
        elif model_type == "backbone_SADC":
            self.backbone       = CSPDarknet_SADC(depth, width, depthwise = depthwise, act = act)
        ...
        #-------------------------------------------#
        #   20, 20, 512 -> 20, 20, 256
        #   lateral_conv0 =Conv2d(512, 256, kernel_size=(1, 1), stride=(1, 1), bias=False)
        #   width = 0.5  ,考虑使用 基准校准通道数，这有啥用，还硬编码in_channels，离谱
        #-------------------------------------------#
    ...
    def forward(self, input):
        if self.training:
            input, clear_x = input.split((input.size(0) // 2, input.size(0) // 2), dim=0)  # split haze and clear images (Batchsize, Batchsize)
        out_features            = self.backbone.forward(input)
        # -------------------------------------------#
        #   feat1 :   torch.Size([8, 128, 80, 80])
        #   feat2 :   torch.Size([8, 256, 40, 40])
        #   feat3 :   torch.Size([8, 512, 20, 20])
        # -------------------------------------------#
        [feat1, feat2, feat3]   = [out_features[f] for f in self.in_features]
        #-------------------------------------------#
        #   20, 20, 512 -> 20, 20, 256
        #   self.lateral_conv0 = Conv2d(512, 256, kernel_size=(1, 1), stride=(1, 1), bias=False)
        #-------------------------------------------#
...
class YoloBody(nn.Module):
    def __init__(self, num_classes, phi, model_type):
        super().__init__()
        depth_dict = {'nano': 0.33, 'tiny': 0.33, 's' : 0.33, 'm' : 0.67, 'l' : 1.00, 'x' : 1.33,}
        width_dict = {'nano': 0.25, 'tiny': 0.375, 's' : 0.50, 'm' : 0.75, 'l' : 1.00, 'x' : 1.25,}
        depth, width    = depth_dict[phi], width_dict[phi]
        depthwise       = True if phi == 'nano' else False
        self.backbone = YOLOPAFPN(model_type=model_type, depth=depth, width=width, depthwise=depthwise)
