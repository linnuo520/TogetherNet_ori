...
if __name__ == "__main__":
    config = Config("original_exp02_noParallel",Parallel = True,unfreeze_epoch=300,unfreeze_batch_size=8,model_type= "backbone_SADC")
    config.print_config_part()
    # 从类的成员属性赋值给外部变量
    config.save_partial_config()

    exp_name = config.exp_name
    train_dataset = config.train_datasets
    if train_dataset == 'voc':
    ...
    model = YoloBody(num_classes, phi,model_type)
    weights_init(model)
    if model_path != '':
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch(model_train, model,exp_name, yolo_loss, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, save_period)
